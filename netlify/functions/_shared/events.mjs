...
import { notifyEventChange, notifyEventDeleted } from './_shared/registrant-notify.mjs';
...
const _handler = async (req) => {
  ...
  if (req.method === 'PUT' && id) {
    ...
    const allowTight = !!ev.allowTight; delete ev.allowTight;
    const allowPend = !!ev.allowPendingOverlap; delete ev.allowPendingOverlap;
    // Transient console hint describing WHAT changed (cancel-occurrence, bump,
    // detach, series-split, restore-occurrence…) — used to word the automatic
    // registrant notifications. Never persisted.
    const changeHint = ev.__change || null; delete ev.__change;
    ...
    if (pendRed.length && !allowPend) return json({ error: 'Overlaps PENDING booking request(s) — ' + describeDated(pendRed), code: 'pending', conflicts: pendRed }, 409);
    if (tight.length && !allowTight) return json({ error: 'Back-to-back (<' + MIN_GAP_MINS + ' min changeover) — ' + describeDated(tight), code: 'tight', conflicts: tight }, 409);
    const prevRows = await sql`SELECT data FROM events WHERE id = ${id}`;
    const prev = prevRows.length ? prevRows[0].data : null;
    await sql`INSERT INTO events (id, data) VALUES (${id}, ${JSON.stringify(ev)}::jsonb)
              ON CONFLICT (id) DO UPDATE SET data = EXCLUDED.data`;
    // Automatic registrant notification (SOP §6.4): email everyone affected by
    // this change. Best-effort — a mail hiccup never fails the save.
    let notifyReport = null;
    if (prev) {
      try { notifyReport = await notifyEventChange(sql, prev, ev, changeHint); }
      catch (e) { console.error('[events] registrant notify failed', e); }
    }
    return json(notifyReport ? Object.assign({}, ev, { _registrantNotify: notifyReport }) : ev);
  }

  if (req.method === 'DELETE' && id) {
    // Before removing the event, cancel + refund + email every active
    // registrant automatically (SOP §6.4), then delete the row.
    const prevRows = await sql`SELECT data FROM events WHERE id = ${id}`;
    if (prevRows.length) {
      try { await notifyEventDeleted(sql, prevRows[0].data); }
      catch (e) { console.error('[events] delete-notify failed', e); }
    }
